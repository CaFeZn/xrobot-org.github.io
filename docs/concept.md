...
所有 I/O 均应建立在无锁队列与环形缓冲之上，运行期不依赖互斥锁或关中断的临界区，从而保证传输路径的确定性与时延可控。设备事件完全由硬件中断驱动，中断仅承担双缓冲切换与状态机切换等必要任务，不涉入额外逻辑，使数据流动严格受硬件节奏控制，不依赖操作系统调度。由此，I/O 可抽象为一条由中断推动的无锁流水线，具备轻量、实时的特性。

为避免事件在回调链中相互触发形成环（例如 A → B → C → A）而产生递归调用、导致栈帧持续累积直至溢出，回调机制在实现上加入了重入保护：当同一回调处于执行状态时，再次触发不形成新的嵌套调用栈帧，而以合并延后等方式处理，从而将调用层级限制在可控范围内。这样可保持栈占用稳定，并使执行路径更易于进行最坏时延评估。
